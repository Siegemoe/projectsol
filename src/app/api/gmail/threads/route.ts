...
import { getUserOAuthClient, gmailFromOAuth, fetchInboxSections } from "@/lib/google/auth";

type Folder = "inbox" | "pinned" | "drafts" | "sent" | "trash";

export async function GET(req: NextRequest) {
  const url = new URL(req.url);
  const folder = (url.searchParams.get("folder") as Folder) || "inbox";
  ...
  // Rate limit per-user
  const rate = RATE_LIMIT_ON ? checkRateLimit(`gmail:list:${user.id}`, 30, 5 * 60 * 1000) : { allowed: true, remaining: 0, reset: Date.now() + 5 * 60 * 1000 };
  ...
  try {
    // Phase 1 (read-only): only "inbox" implemented; other folders return empty sections.
    if (folder !== "inbox") {
      const resp = NextResponse.json([], { status: 200, headers: { ...limitHeaders } });
      for (const op of cookieOps) resp.cookies.set({ name: op.name, value: op.value, ...op.options });
      return resp;
    }
    ...
    const sections = await fetchInboxSections(gmail, 25);
    const resp = NextResponse.json(sections, { status: 200, headers: { ...limitHeaders } });
    for (const op of cookieOps) resp.cookies.set({ name: op.name, value: op.value, ...op.options });
    return resp;
  } catch (e: any) {
    const resp = NextResponse.json({ error: "Failed to fetch threads" }, { status: 500, headers: { ...limitHeaders } });
    for (const op of cookieOps) resp.cookies.set({ name: op.name, value: op.value, ...op.options });
    return resp;
  }
}
