# Adds mcp-knowledge-graph server to existing clean cline_mcp_settings.json
...
# Create complete configuration adding mcp-knowledge-graph
$updatedJson = @'
{
  "mcpServers": {
    ...
    "memory-bank-mcp": {
      ...
      "env": {
        "MEMORY_BANK_ROOT": "C:\\Users\\zghor\\Documents\\Cline\\MemoryBank"
      },
      "disabled": false,
      "autoApprove": ["memory_bank_read", "memory_bank_write", "memory_bank_update", "list_projects", "list_project_files"]
    },
    "mcp-knowledge-graph": {
      "command": "npx",
      "args": ["-y", "mcp-knowledge-graph", "--memory-path", "C:\\Users\\zghor\\Documents\\Cline\\KnowledgeGraph"],
      "env": {},
      "disabled": false,
      "autoApprove": ["aim_create_entities", "aim_create_relations", "aim_add_observations", "aim_search_nodes", "aim_read_graph", "aim_open_nodes", "aim_list_databases", "aim_delete_entities", "aim_delete_observations", "aim_delete_relations"]
    }
  }
}
'@
...
[System.IO.File]::WriteAllText($settingsPath, $updatedJson, (New-Object System.Text.UTF8Encoding($false)))

Write-Host "MCP Knowledge Graph server added to configuration."
Write-Host "Memory path set to: C:\Users\zghor\Documents\Cline\KnowledgeGraph"

# Validate JSON
try {
  $test = $updatedJson | ConvertFrom-Json
